...
class GoogleVeoProvider:
    ...
    async def generate(
        ...
        *,
        ...
        try:
            # Build config (prefer GenerateVideosConfig)
            if hasattr(types, "GenerateVideosConfig"):
                # Build a minimal config. `person_generation` is currently rejected by the API
                # so we omit it to avoid INVALID_ARGUMENT errors.
                config = types.GenerateVideosConfig(
                    aspect_ratio=aspect_ratio,
                )
            else:
                logger.warning(
                    "google-genai `GenerateVideosConfig` not available; using dict fallback. "
                    "Consider upgrading google-genai for full feature support."
                )
                config = {"aspect_ratio": aspect_ratio}

            # Generate (prefer `generate_videos` if present)
            if first_frame_url:
                # Image-to-video
                image_data = await self._fetch_image(first_frame_url)
                # Prefer specific video reference image type if available
                if hasattr(types, "VideoGenerationReferenceImage"):
                    image = types.VideoGenerationReferenceImage(image_bytes=image_data)
                elif hasattr(types, "Image"):
                ...
                if hasattr(client.aio.models, "generate_videos"):
                ...
                elif hasattr(client.aio.models, "generate_video"):
                    ...
                    raise AttributeError("No video generation method available on client.aio.models")
            else:
                # Text-to-video
                if hasattr(client.aio.models, "generate_videos"):
                    operation = await client.aio.models.generate_videos(
